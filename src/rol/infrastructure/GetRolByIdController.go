...
func NewGetRolByIdController(getByIdUseCase *application.GetRolByIdUseCase) *GetRolByIdController {
	return &GetRolByIdController{getByIdUseCase: getByIdUseCase}
}
...
func (ctrl *GetRolByIdController) Run(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"message": "No se pudo obtener el rol", "error": err.Error()})
		return
	}
